...
//2. project_philambda_to_xy: phi2, lambda2  -->  x, y, gamma
        ...
        Mi = GE_WGS84_Alpha * (1.0 - e2) * (M0 * phi2 - M2 * sin(2 * phi2) + M4 * sin(4 * phi2) - M6 * sin(6 * phi2) + M8 * sin(8 * phi2));
...
// utm_project_xy_to_philambda: x, y --> phi2, lambda2, gamma
        ...
        while (fabs(f0 - f0_old) > 1e-17 && acount < 100) {
                ...
                M6 = 0.01139322916666667 * e6 + 0.025634765625 * e8;
                M8 = 0.002408551504771226 * e8;
                Mi = GE_WGS84_Alpha * (1.0 - e2) * (M0 * f0 - M2 * sin(2 * f0) + M4 * sin(4 * f0) - M6 * sin(6 * f0) + M8 * sin(8 * f0));
        ...
        lambda2 = ((((5 + 6 * n2 + 28 * t * t + 8 * t * t * n2 + 24 * t * t * t * t) * P2 / 120 - (1.0 + 2 * t * t + n2) / 6) * P2 + 1.0) * P) / cos(f0) + lambda0;
